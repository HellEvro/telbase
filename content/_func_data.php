...
function func_pursatcom_header_set($afields) //Функция выставления заголовков для таблицы БД массиве
{
    ...
    $afields = str_replace('продолжитть','traff_local',$afields);
    $afields = str_replace('продолж-ть','traff_local',$afields);
    $afields = str_replace('продолжит-ть','traff_local',$afields);
    ...
    //Новый шаблон выгрузки от 09.2013 от оператора.
    $afields = str_replace('шифр','contract',$afields); 
    $afields = str_replace('подразделение','dep',$afields);
    $afields = str_replace('телефон','number',$afields);
    $afields = str_replace('номер','number',$afields);
    $afields = str_replace('№','number',$afields);
    //Новый шаблон выгрузки от 10.2013 от оператора.
    $afields = str_replace('шифр','contract',$afields);
    $afields = str_replace('наименование','dep',$afields);
    $afields = str_replace('№','number',$afields);
    echo 'Ок.';
    return($afields);    
}
...
function func_import_csv($table_name,$filename,$upload_dir,$db,$afields,$company,$regcode)     
{
    ...
    $header = $col_name; //вписываем в массив имена столбцов
...
function func_merge_tables_month($year,$month)     
{
    ...
    $query_create_table = "
        ...
        AUTO_INCREMENT=1;
        ";
        $result_create_table = mysql_query($query_create_table) or die('<br/>Создание таблицы  traffic_'.$table_date.' не удалось, ошибка MySQL: <br/>' . mysql_error());
            unset($query_create_table);
        $query_truncate_table = "TRUNCATE TABLE traffic_".$table_date."";
        $result_truncate_table = mysql_query($query_truncate_table) or die('<br/>Обнуление таблицы  traffic_'.$year.$moths.' не удалось, ошибка MySQL: <br/>' . mysql_error());
            unset($query_truncate_table);
        ////////////////////////////////////////////////////////////////////////
    	$pursatcom_test_query = "SELECT id FROM pursatcom_".$table_date."";
    	$pursatcom_test_result = mysql_query($pursatcom_test_query);
            unset($pursatcom_test_query);
	...
	$rninform_other_test_result = mysql_query($rninform_other_test_query);
        unset($rninform_other_test_query);
	...
	$query_rninform_other = "
		...
		$result_rninform_other = mysql_query($query_rninform_other)or die('<br/>ошибка MySQL: <br/>' . mysql_error());
            unset($query_rninform_other);
    	}
        echo 'Ок.<br />';
////////Запрос на вставку CONTRACT поля в сводную таблицу////////////////////
    $query_insert_contract_data_to_table = "
        UPDATE traffic_".$table_date." as t1 join phone
        SET t1.contract = phone.contract 
        WHERE t1.number = phone.number 
        AND t1.contract IS NULL
    ";
    #print_r($query_insert_contract_data_to_table);
    $contract_data_to_table = mysql_query($query_insert_contract_data_to_table) or die('<br />Внесение ЛС в таблицу traffic_'.$table_date.' ...Неудача!<br />');
        unset($query_insert_contract_data_to_table);
    if ($contract_data_to_table == true)
    {
        echo 'Лицевые счета успешно занесены в БД traffic_',$table_date;
    }
/////////////////////////////////////////////////////////////////////////////  
    return ($result); 
    }
...
////////Запрос на вставку CONTRACT поля в сводную таблицу////////////////////
    $query_insert_contract_data_to_table = "
        UPDATE ".$traffic_table." as t1 join phone
        SET t1.contract = phone.contract 
        WHERE t1.number = phone.number 
        AND t1.contract IS NULL
    ";
    #print_r($query_insert_contract_data_to_table);
    $contract_data_to_table = mysql_query($query_insert_contract_data_to_table) or die('<br />Внесение ЛС в таблицу '.$traffic_table.' ...Неудача!<br />');
     unset($query_insert_contract_data_to_table);
    if ($contract_data_to_table == true)
    {
        echo 'Лицевые счета успешно занесены в БД ',$traffic_table;
    }
/////////////////////////////////////////////////////////////////////////////     
    }
}
...
function func_phone_add()
{
    ...
    if ($tables_names==false)
    {
        echo 'Не найдено ни одной таблиы (за один месяц) за ',$year,' год';
    }
    else
    {
        $query_id = mysql_result(mysql_query("SELECT MAX(id) FROM phone"),0) or die( mysql_error());
